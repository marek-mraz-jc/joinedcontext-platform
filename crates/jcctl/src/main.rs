...
fn main() -> ExitCode {
    ...
    match words.as_slice() {
        ...
        ["checkouts", "--org-dir", org, "--projects-dir", projects, "--forge", forge, rest @ ..] => {
            match checkouts_options(rest) {
                Some((token_file, secrets_dir, once, interval)) => checkouts(
                    Path::new(org),
                    Path::new(projects),
                    &jcctl::commands::checkouts::Options {
        ...
        ["import", "--format", "git", source, "--out-dir", out] => {
            git_import(Path::new(source), Path::new(out))
        }
        ["export", "--format", "git", rest @ ..] => match git_export_options(rest) {
            Some((repo_dir, project, out, apps, org_dir)) => {
                git_export(&repo_dir, &project, &apps, org_dir.as_deref(), &out)
            }
            None => usage(),
        },
        ...
        ["model", verb, rest @ ..] => match (model_mode(verb), model_options(rest)) {
            (Some(mode), Some((dir, org_dir, url))) => model(&dir, org_dir.as_deref(), url, mode),
            _ => usage(),
        },
        ...
        ["login", rest @ ..] => match client_options(rest) {
            Some(options)
...
fn portal(options: &ClientOptions) -> Result<jcctl::portal::Portal, String> {
    ...
    // `jcctl login` writes the default file; a named one wins over it.
    let token_file = options
        .token_file
        .map(PathBuf::from)
        .or_else(|| non_empty("JC_TOKEN_FILE").map(PathBuf::from))
        .or_else(|| jcctl::login::default_token_file().filter(|path| path.is_file()))
        .ok_or("no identity: run jcctl login, or pass --token-file <path> or set JC_TOKEN_FILE, a file holding an OIDC access token for the Portal")?;
    let token = jcctl::gateway::Gateway::token_from(&token_file).map_err(|e| e.to_string())?;
...
fn login(options: &ClientOptions) -> ExitCode {
    let Some(idm) = options
        .idm
        .map(str::to_owned)
        .or_else(|| non_empty("JC_IDM"))
    else {
        return fail("no identity provider: pass --idm <issuer url>, e.g. https://idm.<domain>/realms/<realm>, or set JC_IDM");
    };
    let issuer = match jcctl::login::issuer(&idm) {
        Ok(issuer) => issuer,
    ...
    let Some(path) = options
        .token_file
        .map(PathBuf::from)
        .or_else(jcctl::login::default_token_file)
    else {
        return fail(
            "no place for the token: pass --token-file <path>, or set HOME or XDG_CONFIG_HOME",
        );
    };
    match jcctl::login::login(
        &issuer,
        &path,
        |line| eprintln!("{line}"),
        std::thread::sleep,
        ...
        Ok(done) => {
            match done.expires_in {
                Some(seconds) => println!(
                    "signed in; token written to {} (valid {seconds}s)",
                    done.path.display()
                ),
                None => println!("signed in; token written to {}", done.path.display()),
            }
            ExitCode::SUCCESS
        }
...
fn client(args: &[&str]) -> ExitCode {
    ...
    if !shape_fits || options.idm.is_some() || (!reads && options.output.is_some()) {
        return usage();
    }
...
/// `--repo-dir`, `--project`, `--out-dir`, any number of `--app-dir name=path` (MF-45) and the
/// `--org-dir` the imported organization models are read from (MF-49).
...
fn git_export_options(args: &[&str]) -> Option<GitExportOptions> {
    let (mut repo_dir, mut project, mut out, mut apps, mut org_dir) =
        (None, None, None, BTreeMap::new(), None);
    let mut rest = args;
    while let [flag, value, tail @ ..] = rest {
        match *flag {
            "--repo-dir" => repo_dir = Some(PathBuf::from(value)),
            ...
            "--org-dir" => org_dir = Some(PathBuf::from(value)),
            "--app-dir" => {
                let (name, dir) = value.split_once('=')?;
                apps.insert(name.to_owned(), PathBuf::from(dir));
            }
            _ => return None,
        }
        rest = tail;
...
/// A whole project as git bundles and their index (MF-45).
fn git_export(
    repo_dir: &Path,
    project: &str,
    ...
    match jcctl::commands::git_bundle::export(repo_dir, project, apps, org_dir, out, &exported_by) {
        Ok(index) => {
            for repository in &index.spec.repositories {
                println!(
                    "{} {} at {}",
                    repository.file, repository.name, repository.head
                );
            }
            ExitCode::SUCCESS
        }
...
/// Every bundle of a git-native export cloned and checked against its index (MF-46, MF-47).
fn git_import(source: &Path, out: &Path) -> ExitCode {
    match jcctl::commands::git_bundle::import(source, out) {
        Ok(imported) => {
            for (name, path, head) in &imported.repositories {
                println!("{name}: {} at {head}, equal", path.display());
            }
            ExitCode::SUCCESS
        }
        Err(err) => {
...
type CheckoutsOptions = (Option<PathBuf>, Option<PathBuf>, bool, u64);

fn checkouts_options(mut rest: &[&str]) -> Option<CheckoutsOptions> {
    let (mut token_file, mut secrets_dir, mut once, mut interval) = (None, None, false, 10);
    loop {
        rest = match rest {
            [] => return Some((token_file, secrets_dir, once, interval)),
            ["--token-file", file, more @ ..] => {
                token_file = Some(PathBuf::from(file));
                more
            }
            ["--secrets-dir", dir, more @ ..] => {
                secrets_dir = Some(PathBuf::from(dir));
                more
            }
            ["--once", more @ ..] => {
                once = true;
            ...
            ["--interval", seconds, more @ ..] => {
                interval = seconds.parse().ok().filter(|s| *s > 0)?;
                more
            }
            _ => return None,
        };
...
/// `jcctl checkouts`: one pass, or one pass every `interval` seconds until the process ends.
/// A project whose fetch fails keeps its last checkout and is reported; `--once` exits non-zero
/// only when the registry itself does not read, so one unreachable project never keeps the
/// gateway from starting with the others.
fn checkouts(
    org: &Path,
    projects: &Path,
    options: &jcctl::commands::checkouts::Options,
    once: bool,
    interval: u64,
) -> ExitCode {
    loop {
        let mut failed = false;
        match jcctl::commands::checkouts::sync(org, projects, options) {
            Ok(linked) => {
                for entry in linked {
                    match entry.result {
                        Ok(tree) if entry.moved => {
                            println!("project {}: {}", entry.slug, tree.display());
                        }
                        Ok(_) => {}
                        Err(err) => {
                            eprintln!("project {}: kept the last checkout: {err}", entry.slug);
                        }
                    }
                }
            }
            Err(err) => {
                failed = true;
                eprintln!("the registry did not read: {err}");
            }
        }
        if once {
            return if failed {
...
fn model(dir: &Path, org_dir: Option<&Path>, url: Option<String>, mode: model::Mode) -> ExitCode {
    ...
    let report = match model::run(dir, org_dir, &tools, mode) {
        Ok(report) => report,
        Err(err) => return fail(&err.to_string()),
    };
...
/// Parses `--repo-dir` and the optional `--org-dir` and `--url`, in any order.
fn model_options(args: &[&str]) -> Option<(PathBuf, Option<PathBuf>, Option<String>)> {
    let (mut dir, mut org_dir, mut url) = (None, None, None);
    let mut rest = args;
    while let [flag, value, tail @ ..] = rest {
        match *flag {
